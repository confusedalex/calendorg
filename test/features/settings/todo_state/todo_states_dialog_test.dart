...
void main() {
  ...
  group(
    "todo_states_dialog_test",
    () {
      group("finding chips", () {
        testWidgets(
          'should find one todo chip',
          (tester) async {
            await pumpWidgetToTester(tester);
            await tester.pumpAndSettle();

            final todoChip = find.byWidgetPredicate((widget) =>
                widget is Chip && (widget.label as Text).data == "TODO");

            expect(todoChip, findsOne);
          },
        );
        testWidgets(
          'should find one done chip',
          (tester) async {
            await pumpWidgetToTester(tester);
            await tester.pumpAndSettle();

            final doneChip = find.byWidgetPredicate((widget) =>
                widget is Chip && (widget.label as Text).data == "DONE");
      ...
      testWidgets(
        'tapping on first plus button will open dialog',
        (tester) async {
          await pumpWidgetToTester(tester);
          await tester.pumpAndSettle();
      ...
      testWidgets(
        'tapping on second plus button will open dialog',
        (tester) async {
          await pumpWidgetToTester(tester);
          await tester.pumpAndSettle();
